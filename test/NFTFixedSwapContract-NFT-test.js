...
describe("Fixed Swap Contract - NFT", function () {
  ...
  beforeEach(async function () {
    ...
    contractRadaFixedSwap = await upgrades.deployProxy(NFTFixedSwapContract, [bUSDToken.address], {
      kind: 'uups'
    });
    ...
    for (let i=tokenIdStart;i<=tokenIdEnd;i++) {
      await contractNFT.safeMint(contractRadaFixedSwap.address, i);
      saleTokenIds.push(i);
    }
    ...
    await contractRadaFixedSwap.updatePool(poolId, addressItem, saleTokenIds, startTime, endTime, priceEach, requireWhitelist, maxBuyPerAddress);
  ...
  it('Should place order successfully - whitelist', async function () {
    ...
    await contractRadaFixedSwap.handlePublicPool(poolId, false);
    await contractRadaFixedSwap.updatePool(poolId, pool.addressItem, pool.saleTokenIds, pool.startTime, pool.endTime, pool.startPrice, pool.requireWhitelist, maxBuyPerAddress);
    await contractRadaFixedSwap.handlePublicPool(poolId, true);
  ...
  it('Should place order successfully - public', async function () {
    ...
    const maxBuyPerAddress = 10;
    await contractRadaFixedSwap.updatePool(poolId, pool.addressItem, pool.saleTokenIds, pool.startTime, pool.endTime, pool.startPrice, requireWhitelist, maxBuyPerAddress);
    await contractRadaFixedSwap.handlePublicPool(poolId, true);
  ...
  it('Should place order successfully & Claimed - public', async function () {
    ...
    const requireWhitelist = false;
    await contractRadaFixedSwap.handlePublicPool(poolId, false);
    await contractRadaFixedSwap.updatePool(poolId, pool.addressItem, pool.saleTokenIds, pool.startTime, pool.endTime, pool.startPrice, requireWhitelist, pool.maxBuyPerAddress);
    await contractRadaFixedSwap.handlePublicPool(poolId, true);
  ...
  it('Should reverted place order when pool has been not start or expired - whitelist', async function () {
    ...
    await contractRadaFixedSwap.handlePublicPool(poolId, false);
    await contractRadaFixedSwap.updatePool(poolId, pool.addressItem, pool.saleTokenIds, timeNotStart, pool.endTime, pool.startPrice, pool.requireWhitelist, pool.maxBuyPerAddress);
    await contractRadaFixedSwap.handlePublicPool(poolId, true);
    // Should reverted
    ...
    await contractRadaFixedSwap.handlePublicPool(poolId, false);
    await contractRadaFixedSwap.updatePool(poolId, pool.addressItem, pool.saleTokenIds, timeStart, pool.endTime, pool.startPrice, pool.requireWhitelist, pool.maxBuyPerAddress);
    await contractRadaFixedSwap.handlePublicPool(poolId, true);
    // Now
    // Bought success
    await contractRadaFixedSwap.connect(buyerUser).placeOrder(poolId, quantity);
